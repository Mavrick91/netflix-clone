		...
		<div className="relative ml-5 flex w-full lg:ml-10">
			<button
				...
				className={classNames(
					"text-white border transition-all relative border-white pr-7 text-[9px] lg:pr-14 h-auto  lg:py-1 pl-2 lg:font-bold text-sm lg:text-lg bg-black hover:bg-transparent",
					{
				...
				>
					<ul className="grid grid-cols-1 py-2 lg:grid-cols-3">
						{items.map((item) => (
							<LinkComponent
								href={`${item.id}`}
								className="col-span-1 whitespace-nowrap px-3 py-1 underline-offset-2 hover:underline"
								key={item.id}
							>
								{item.name}
							</LinkComponent>
